...
it("should render card component(s) when cityList has elements", () => {
  const wrapper = shallow(<DisplayCities {...props} />);
  expect(wrapper.find(Card).length).toBe(props.cityList.length);
});

it("should not render card component(s) when cityList is empty", () => {
  const localProps = {
    ...props,
    cityList: []
  };
